...
func TestPerspectivesList(t *testing.T) {
	var perspectiveTests = []struct {
		...
		{"Invalid option", "test_data/empty.txt", []int{6, 3}, []FuncOption{SetOffset(9999)}, ErrOutOfRange.Error()},
...
func TestPerspectivesSearch(t *testing.T) {
	var perspectiveTests = []struct {
		...
		{"Invalid option", "test_data/empty.txt", "person", []FuncOption{SetOffset(9999)}, ErrOutOfRange.Error()},
