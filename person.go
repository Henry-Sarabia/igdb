...
// retrieve. If the ID does not match any Persons, an error is returned.
...
// List returns a list of Persons identified by the provided list of IGDB IDs.
...
func (ps *PersonService) List(ids []int, opts ...Option) ([]*Person, error) {
	...
	err := ps.client.get(ps.end, &p, opts...)
	if err != nil {
		return nil, errors.Wrapf(err, "cannot get Persons with IDs %v", ids)
	}

	return p, nil
}

// Index returns an index of Persons based solely on the provided functional
// options used to sort, filter, and paginate the results. If no Persons can
...
func (ps *PersonService) Index(opts ...Option) ([]*Person, error) {
	...
	err := ps.client.get(ps.end, &p, opts...)
	if err != nil {
		return nil, errors.Wrap(err, "cannot get index of Persons")
	}

	return p, nil
}

// Count returns the number of Persons available in the IGDB.
// Provide the SetFilter functional option if you need to filter
// which Persons to count.
func (ps *PersonService) Count(opts ...Option) (int, error) {
	ct, err := ps.client.getCount(ps.end, opts...)
	if err != nil {
		return 0, errors.Wrap(err, "cannot count Persons")
	}
