...
func TestPlatformsList(t *testing.T) {
	var platformTests = []struct {
		...
		{"Invalid option", "test_data/empty.txt", []int{23, 130}, []FuncOption{SetOffset(9999)}, ErrOutOfRange.Error()},
...
func TestPlatformsSearch(t *testing.T) {
	var platformTests = []struct {
		...
		{"Invalid option", "test_data/empty.txt", "xbox", []FuncOption{SetOffset(9999)}, ErrOutOfRange.Error()},
